...
function parseArgs(args: string[]): CliArgs {
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        if (arg === '--allow-layout-fork') {
            parsed.allowLayoutFork = true;
            continue;
        }
        if (arg === '--lesson-id') {
            ...
            for (const course of accessible) {
                try {
                    await downloadCourse({
                        ...
                        update: args.update,
                        forceUpdate: args.forceUpdate,
                        allowLayoutFork: args.allowLayoutFork
                    });
                } catch (err) {
                    failedCourses += 1;
